...
select * , rank() over( partition by  order_id, customer_id order by quantity desc) 'rank' from orders;

-- 5) Find the running total (cumulative sum) of the price for each customer, sorted by order_date:
select customer_id, order_date, price, sum(price) over (partition by customer_id order by order_date) 'cum_price' from orders;